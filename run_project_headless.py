...
def read_project_nguon_metadata_cache(project_dir: Path, code: str) -> dict:
    ...
    if reference_channel:
        root = SRT_TOOL_DIR / "reference_characters" / "psychology" / reference_channel
...
def resolve_psychology_reference_channel(value: str, project_code: str = "") -> str:
    """Resolve project codes like TL1-0002 to reference folders like TL1-T2."""
    candidates = []
    for item in [value, project_code]:
        item = str(item or "").strip()
        if item and item not in candidates:
            candidates.append(item)
        m = re.match(r"^(TL\d+)-0*(\d+)$", item, flags=re.IGNORECASE)
        if m:
...
def apply_project_runtime_metadata(cfg: dict, project_dir: Path, code: str) -> dict:
    ...
    if cfg.get("topic") == "psychology":
        ref = Path(str(nguon_meta.get("psychology_reference_image") or ""))
        if not ref.exists():
            ref = SRT_TOOL_DIR / "reference_characters" / "psychology" / str(cfg.get("reference_channel") or code) / "nv1.png"
        if ref.exists():
