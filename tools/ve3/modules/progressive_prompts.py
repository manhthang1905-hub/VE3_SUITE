...
class ProgressivePromptsGenerator:
    ...
    def __init__(self, config: dict):
        ...
        if one_key:
            self.deepseek_keys.append(one_key)
        for key in many_keys:
            key = (key or "").strip()
            if key and key not in self.deepseek_keys:
        ...
        self.vov_direct_model = (config.get("vov_direct_model", "") or "claude-opus-4-6").strip()
        ...
        claude_chain_cfg = config.get("claude_pool_model_chain", []) or []
        if isinstance(claude_chain_cfg, str):
            claude_chain_cfg = [x.strip() for x in claude_chain_cfg.split(",") if x.strip()]
        self.claude_pool_model_chain = [str(x).strip() for x in claude_chain_cfg if str(x).strip()]
    ...
    def _resolve_ai_provider(self) -> str:
        ...
        if provider in ("deepseek", "vov_direct", "claude_pool"):
            return provider
        if self.vov_direct_base_url and self.vov_direct_api_key and self.vov_direct_model:
            return "vov_direct"
        if self.claude_pool_base_url and self.claude_pool_api_key and self.claude_pool_model:
            return "claude_pool"
        return "deepseek"

    def _provider_sequence(self) -> List[str]:
        if self.ai_provider == "vov_direct":
            return ["vov_direct", "claude_pool", "deepseek"]
        if self.ai_provider == "claude_pool":
            return ["claude_pool", "vov_direct", "deepseek"]
        return ["deepseek", "vov_direct", "claude_pool"]
    ...
    def _call_api(self, prompt: str, temperature: float = 0.7, max_tokens: int = 8192) -> Optional[str]:
        """
        Goi AI theo provider uu tien.
        Khi chon DeepSeek, he thong se tu fallback sang VOV Direct roi Claude Pool neu can.
        """
        providers = self._provider_sequence()
        for idx, provider_name in enumerate(providers):
            if provider_name == "deepseek":
                content = self._call_deepseek_api(prompt, temperature=temperature, max_tokens=max_tokens)
            elif provider_name == "vov_direct":
                content = self._call_vov_direct_api(prompt, temperature=temperature, max_tokens=max_tokens)
            else:
                content = self._call_claude_pool_api(prompt, temperature=temperature, max_tokens=max_tokens)

            if content:
                if idx > 0:
                    self._log(f"  Fallback thanh cong voi {provider_name}", "INFO")
                return content

            if idx < len(providers) - 1:
                self._log(f"  {provider_name} khong kha dung, thu provider tiep theo", "WARN")

        self._log("  ERROR: Tat ca provider AI deu that bai!", "ERROR")
        return None

    def _call_deepseek_api(self, prompt: str, temperature: float = 0.7, max_tokens: int = 8192) -> Optional[str]:
        import requests
        import time

        if not self.deepseek_keys:
            self._log("  DeepSeek: khong co API key", "WARN")
            return None
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                if resp.status_code == 200:
                    if attempt > 0:
                        self._log(f"  DeepSeek success after {attempt + 1} attempts", "INFO")
                ...
                if resp.status_code == 429:
                    ...
                    self._log(f"  DeepSeek rate limit (429), retry {attempt + 1}/{max_retries} after {delay}s", "WARN")
                ...
                if resp.status_code >= 500:
                    delay = base_delay * (2 ** attempt)
                    self._log(f"  DeepSeek server error ({resp.status_code}), retry {attempt + 1}/{max_retries} after {delay}s", "WARN")
                    if attempt < max_retries - 1:
                        time.sleep(delay)
                        continue
                    return None

                self._log(f"  DeepSeek error: {resp.status_code} - {resp.text[:200]}", "ERROR")
                return None

            except requests.exceptions.Timeout:
                delay = base_delay * (2 ** attempt)
                self._log(f"  DeepSeek timeout, retry {attempt + 1}/{max_retries} after {delay}s", "WARN")
                if attempt < max_retries - 1:
                    time.sleep(delay)
                    continue
                return None
                ...
                delay = base_delay * (2 ** attempt)
                self._log(f"  DeepSeek exception: {e}, retry {attempt + 1}/{max_retries} after {delay}s", "WARN")
                if attempt < max_retries - 1:
                    time.sleep(delay)
                    continue
                return None
    ...
    def _call_vov_direct_api(self, prompt: str, temperature: float = 0.7, max_tokens: int = 8192) -> Optional[str]:
        import requests

        if not (self.vov_direct_base_url and self.vov_direct_api_key and self.vov_direct_model_chain):
            self._log("  VOV Direct: thieu cau hinh", "WARN")
            return None
        ...
        max_attempts = max(1, int(self.config.get("excel_ai_attempts_per_key", 3) or 3))

        for model_name in self.vov_direct_model_chain:
            data = {
                "model": model_name,
                "messages": [{"role": "user", "content": prompt}],
                "temperature": temperature,
            ...
            for _ in range(max_attempts):
                try:
                    ...
                    if resp.status_code in (408, 409, 429, 500, 502, 503, 504):
                        self._log(f"  VOV {model_name} loi tam thoi {resp.status_code}, se thu lai", "WARN")
                        continue
                    self._log(f"  VOV {model_name} loi {resp.status_code}: {resp.text[:200]}", "WARN")
                    break
                except Exception as e:
                    self._log(f"  VOV {model_name} exception: {e}", "WARN")
        return None

    def _call_claude_pool_api(self, prompt: str, temperature: float = 0.7, max_tokens: int = 8192) -> Optional[str]:
        import requests

        if not (self.claude_pool_base_url and self.claude_pool_api_key and self.claude_pool_model_chain):
            self._log("  Claude Pool: thieu cau hinh", "WARN")
            return None
        ...
        url = f"{self.claude_pool_base_url}/chat/completions"
        timeout_seconds = int(self.config.get("excel_ai_timeout_seconds", 180) or 180)

        for model_name in self.claude_pool_model_chain:
            data = {
                "model": model_name,
                "messages": [{"role": "user", "content": prompt}],
                "temperature": temperature,
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if content:
                        return content
                self._log(f"  Claude Pool {model_name} loi {resp.status_code}", "WARN")
            except Exception as e:
                self._log(f"  Claude Pool {model_name} exception: {e}", "WARN")
        return None
...
{end_text}"""
    ...
    def step_analyze_story_segments(
        ...
        # v1.0.455: Bỏ logic mode - chỉ giữ 1 logic duy nhất (5-8s/scene cho tất cả)
        # =====================================================================
        self._log(f"\n  [PHASE 2] Calculating image_count for {len(segments)} segments (5-8s/scene)...")
        ...
        def _calculate_image_count_for_segment(seg_with_idx):
            ...
            # v1.0.455: Bỏ logic mode - luôn áp dụng 5-8s/scene cho TẤT CẢ segments
    ...
    "reasoning": "Brief explanation (optional)"
            ...
            if calc_response:
                ...
                if calc_data and "image_count" in calc_data:
                    ...
                    # Fallback: Use target
                    return (idx, target_images, srt_count, "fallback")
            else:
...
"""
        ...
        # v1.0.455: Luôn check global count cho tất cả projects
        # =====================================================================
        if segments:
...
"""
    ...
    def step_create_director_plan_basic(
        ...
        def process_segment_basic(seg_idx_seg):
            ...
            # v1.0.455: Luôn áp dụng 8s rule cho TẤT CẢ segments
            ...
            # v1.0.452: FIX - Lấy srt_start, srt_end, duration từ SRT entries thực tế
            # API tự sinh ra các giá trị này không chính xác → phải map từ srt_indices
            for sc in api_scenes:
                sc_indices = sc.get("srt_indices", [])
                if isinstance(sc_indices, list) and sc_indices:
                    # Lấy SRT entries tương ứng với indices
                    sc_ents = [e for idx, e in enumerate(srt_entries, 1) if idx in sc_indices]

                    if sc_ents:
                        # Populate srt_text nếu thiếu
                        if not sc.get("srt_text"):
                            sc["srt_text"] = " ".join([e.text for e in sc_ents])

                        # FIX: Luôn lấy srt_start và srt_end từ SRT entries thực tế
                        sc["srt_start"] = sc_ents[0].start_time
                        sc["srt_end"] = sc_ents[-1].end_time

                        # FIX: Tính lại duration từ timestamps thực tế
                        try:
                            start_sec = parse_srt_timestamp(str(sc_ents[0].start_time))
                            end_sec = parse_srt_timestamp(str(sc_ents[-1].end_time))
                            if end_sec > start_sec:
                                sc["duration"] = round(end_sec - start_sec, 2)
                        except:
                            pass  # Giữ nguyên duration từ API nếu parse lỗi
        ...
        # v1.0.455: Bỏ logic mode - luôn chạy split/merge cho TẤT CẢ scenes
        # =====================================================================
        ...
        # v1.0.455: Luôn chạy split cho TẤT CẢ scenes > max_dur
        split_count = 0
        split_result = []
        for scene in all_scenes:
            ...
            if dur > max_dur:
                ...
                # v1.0.454: FIX - Nếu có srt_indices, dùng logic cũ
                if isinstance(indices, list) and len(indices) >= n_parts:
                    ...
                    # v1.0.454: FIX - Nếu KHÔNG có srt_indices, chia dựa trên duration
                    # Tính target duration cho mỗi sub-scene
                    target_dur = dur / n_parts

                    # Chia scene thành n_parts dựa trên duration
                    for p in range(n_parts):
                        sub_scene = dict(scene)  # shallow copy
                        sub_scene["duration"] = round(target_dur, 2)

                        # Giữ nguyên srt_text (không thể chia chính xác mà không có srt_indices)
                        # Đánh dấu là sub-scene
                        sub_scene["visual_moment"] = f"[Split {p+1}/{n_parts}] {scene.get('visual_moment', '')}"

                        split_result.append(sub_scene)
                    split_count += 1
                    self._log(f"     -> Scene {scene.get('scene_id')} ({dur}s) split into {n_parts} parts (no srt_indices, duration-based)", "WARNING")
            else:
        ...
        # v1.0.455: Luôn chạy merge cho TẤT CẢ scenes < min_dur
        total_merge_count = 0
...
"""
        ...
        for batch_num in sorted(batch_results.keys()):
            ...
            for scene_data in api_scenes:
                ...
                if is_video_only:
                    ...
                    # else: giữ char_ids từ fallback (line 3295) - quan trọng cho psychology topic
                ...
                # v1.0.455: Bỏ logic mode - không skip video generation
                video_note = ""
    ...
    def step_8_thumbnail_prompts(
        ...
        characters = workbook.get_characters()
        locations = workbook.get_locations()
        existing_thumbnails = workbook.get_thumbnails() or []
        ...
        # Thumbnail generation later depends on actual usable media references.
        # Keep location refs only when they already have a resolvable media_id path.
        char_ids = [c.id for c in main_chars[:5]]
        available_media_ids = {}
        try:
            available_media_ids = workbook.get_media_ids() or {}
        except Exception:
            available_media_ids = {}
        loc_ids = []
        if locations:
            for loc in locations[:5]:
                loc_id = getattr(loc, "id", "")
                if loc_id and available_media_ids.get(loc_id):
                    loc_ids.append(loc_id)

        thumbnail_text_guidance = ""
        if existing_thumbnails:
            guidance_lines = []
            for idx, thumb in enumerate(existing_thumbnails[:3], start=1):
                raw_prompt = str(getattr(thumb, "img_prompt", "") or "")
                if not raw_prompt:
                    continue
                text_match = re.search(
                    r"Render on-image text EXACTLY as this content only:\s*(.+?)(?:\s+Do NOT render|\s+Highlight words:|$)",
                    raw_prompt,
                    flags=re.IGNORECASE | re.DOTALL
                )
                highlight_match = re.search(
                    r"Highlight words:\s*(.+?)(?:\s+Font:|$)",
                    raw_prompt,
                    flags=re.IGNORECASE | re.DOTALL
                )
                text_value = re.sub(r"\s+", " ", text_match.group(1)).strip(" .") if text_match else ""
                highlight_value = re.sub(r"\s+", " ", highlight_match.group(1)).strip(" .") if highlight_match else ""
                if text_value:
                    guidance_lines.append(
                        f"- Thumb {idx}: keep this exact on-image text: {text_value}"
                    )
                    if highlight_value:
                        guidance_lines.append(
                            f"  Highlight words to preserve: {highlight_value}"
                        )
            if guidance_lines:
                thumbnail_text_guidance = "\n".join(guidance_lines)
        ...
        valid_loc_ids = set(loc_ids)

        def _sanitize_thumbnail_item(item: dict) -> dict:
            import re

            clean = dict(item or {})
            img_prompt = str(clean.get("img_prompt", "") or "").strip()
            img_prompt_portrait = str(clean.get("img_prompt_portrait", "") or "").strip()
            chars_used = str(clean.get("characters_used", "") or "").strip()
            loc_used = str(clean.get("location_used", "") or "").strip()

            def _strip_invalid_refs(text: str) -> str:
                if not text:
                    return text

                def _repl(match):
                    ref_id = (match.group(1) or "").strip()
                    if ref_id in valid_char_ids or ref_id in valid_loc_ids:
                        return match.group(0)
                    return ""

                text = re.sub(r"\(([A-Za-z0-9_]+)\.png\)", _repl, text)
                text = re.sub(r"\s{2,}", " ", text)
                text = re.sub(r"\s+([,.;:])", r"\1", text)
                return text.strip()

            if chars_used:
                chars_filtered = [cid.strip() for cid in chars_used.split(",") if cid.strip() in valid_char_ids]
                chars_used = ", ".join(chars_filtered)
            if not chars_used and valid_char_ids:
                chars_used = next(iter(valid_char_ids))

            if loc_used:
                loc_filtered = [lid.strip() for lid in loc_used.split(",") if lid.strip() in valid_loc_ids]
                loc_used = ", ".join(loc_filtered)

            clean["characters_used"] = chars_used
            clean["location_used"] = loc_used
            clean["img_prompt"] = _strip_invalid_refs(img_prompt)
            clean["img_prompt_portrait"] = _strip_invalid_refs(img_prompt_portrait)
            return clean

        saved = 0
        for item in thumbnails_data[:3]:
            item = _sanitize_thumbnail_item(item)
            ...
            # Keep declared character/location usage even if the model omitted a token
            # inside the prompt text, so downstream thumbnail generation preserves identity.
            ref_files = []
            for cid in [c.strip() for c in chars_used.split(",") if c.strip()]:
                ref_files.append(f"nv/{cid}.png")
            for lid in [l.strip() for l in loc_used.split(",") if l.strip()]:
