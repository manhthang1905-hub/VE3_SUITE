...
class VideoModel(Enum):
    ...
    VEO3_T2V_LITE = "veo_3_1_t2v_lite_low_priority"
    VEO3_FAST = "veo_3_1_t2v_lite_low_priority"
    VEO3_QUALITY = "veo_3_1_t2v"
    # Image-to-Video models (r2v = reference to video)
    VEO3_I2V_LITE = "veo_3_1_r2v_lite_low_priority"
    VEO3_I2V_FAST_LANDSCAPE = "veo_3_1_r2v_lite_low_priority"
    VEO3_I2V_FAST_PORTRAIT = "veo_3_1_r2v_lite_low_priority"
    VEO3_I2V_QUALITY_LANDSCAPE = "veo_3_1_r2v_lite_low_priority"
...
class GoogleFlowAPI:
    ...
    def _generate_images_via_proxy(
        ...
        proxy_payload = {
            ...
            "job_id": client_job_id or f"img-{uuid.uuid4()}",
        }
        ...
        try:
            ...
            job_id = result.get("jobId") or proxy_payload["job_id"]

            self._log(f"Task created: {task_id} | job={job_id}")

            # Step 2: Poll for result
            return self._poll_proxy_task(task_id, job_id, prompt, aspect_ratio, proxy_headers,
                                         poll_callback=poll_callback)
    ...
    def _poll_proxy_task(
        ...
        """
        self._log(f"Polling task {task_id} (job={job_id})...")
        ...
        STUCK_GIVEUP = 2400      # v1.0.701: 40 phut stuck moi bo cuoc (server image co the lau)
        ...
        while True:
            ...
            try:
                response = requests.get(
                    f"{self.PROXY_TASK_STATUS_URL}?taskId={task_id}&jobId={job_id or ''}",
                    headers=headers,
                    timeout=30
                )
                ...
                if queue_position is not None:
                    if last_position is None or queue_position != last_position:
                        ...
                        last_position_change = time.time()
                        # Report queue position to callback
                        if poll_callback:
                            try:
                                poll_callback({"queue_position": queue_position, "status": task_status or current_status, "elapsed": int(elapsed)})
                            except Exception:
                                pass
                ...
                if task_result.get("success") == True:
                    ...
                    if "unusual activity" in err_lower or "help center" in err_lower:
                        return False, [], f"Task failed: 403 unusual activity block - {str(error)[:200]}"
    ...
    def generate_video(
        ...
        # For Image-to-Video, auto-select correct model per aspect ratio
        if is_i2v and model in [VideoModel.VEO3_FAST, VideoModel.VEO3_QUALITY,
                                 VideoModel.VEO3_I2V_FAST, VideoModel.VEO3_I2V_QUALITY]:
            if aspect_ratio == VideoAspectRatio.PORTRAIT:
                model = VideoModel.VEO3_I2V_FAST_PORTRAIT
            else:
                model = VideoModel.VEO3_I2V_FAST_LANDSCAPE
            self._log(f"Image-to-Video model: {model.value}")

        # Build request payload — đúng format Google API
        request_data = {
            "aspectRatio": aspect_ratio.value,
            "seed": seed,
            "textInput": {
        ...
        if recaptcha_token:
            client_ctx["recaptchaToken"] = recaptcha_token

        # Add referenceImages vào request_data (server cần đọc mediaId từ đây)
        if reference_image_id:
            request_data["referenceImages"] = [{
                "imageUsageType": "IMAGE_USAGE_TYPE_ASSET",
                "mediaId": reference_image_id
            }]
            self._log(f"I2V reference image: {reference_image_id[:50]}...")
        ...
        # Chọn endpoint: local server / proxy / direct
        if self.use_proxy and (self.local_server_url or self.proxy_api_token):
            return self._generate_video_via_proxy(payload, prompt, seed, scene_id, client_job_id=client_job_id)
        else:
    ...
    def _generate_video_via_proxy(
        ...
        """
        is_local = bool(self.local_server_url)
        if not is_local and not self.proxy_api_token:
            return False, VideoGenerationResult(
                status="failed",
                prompt=prompt,
                seed=seed,
        ...
        if is_i2v:
            ...
            self._log(f"Image-to-Video: mediaId={mid[:40]}")

        proxy_payload = {
            "body_json": payload,
            "flow_auth_token": self.bearer_token,
            "flow_url": flow_url,
            "job_id": client_job_id or f"vid-{uuid.uuid4()}",
        }

        # Debug logging
        self._log(f"=== VIDEO PROXY REQUEST ===")
        self._log(f"flow_url: {proxy_payload['flow_url']}")
        self._log(f"body_json: {json.dumps(payload)[:500]}")
        ...
        try:
            ...
            job_id = result.get("jobId") or proxy_payload["job_id"]
            self._log(f"Video task created: {task_id} | job={job_id}")

            # Step 2: Poll for result
            return self._poll_proxy_video_task(task_id, job_id, prompt, seed, scene_id, proxy_headers)
    ...
    def _poll_proxy_video_task(
        ...
        """
        Poll proxy video task — chờ server hoàn tất toàn bộ lifecycle.
        PERSISTENT: chờ cho đến khi server trả kết quả (giống ảnh).

        Server xử lý toàn bộ: submit Google → poll Google → trả video URL.
        Client chỉ cần poll proxy task-status cho đến khi:
        - Video SUCCESSFUL → lấy URL
        - Video FAILED → báo lỗi
        - Stuck quá lâu → timeout

        Fallback: Nếu proxy trả operations nhưng không tự poll Google,
        client sẽ thử poll Google trực tiếp (nhưng có thể bị 403).
        """
        ...
        def _deep_find_url(obj):
            ...
            if isinstance(obj, str):
                if obj.startswith(("http://", "https://")) and any(
                    hint in obj for hint in (".mp4", "video", "fife", "download", "lh3.", "ugc/")
                ):
                    return obj
                return None
            if isinstance(obj, dict):
                for key in ("fifeUrl", "signedUrl", "downloadUrl", "downloadURL",
                            "videoUrl", "videoURL", "url", "uri", "video_url"):
                    val = obj.get(key)
                    found = _deep_find_url(val)
                    if found:
                        return found
        ...
        def _check_operations_status(ops):
            """Kiểm tra status và URL từ operations array.
            Returns: (is_done, is_failed, video_url, error_msg)
            """
            if not ops:
            ...
            if "SUCCESSFUL" in op_status or is_done:
                video_url = _deep_find_url(op)
                return True, False, video_url, ""

            # FAILED
            if "FAILED" in op_status or "ERROR" in op_status:
                err = op.get("error", op_status)
                if isinstance(err, dict):
                    err = err.get("message", str(err))
                return False, True, None, str(err)

            # Still PENDING / PROCESSING
            return False, False, None, ""

        def _check_media_status(task_result):
            """Kiểm tra status và URL từ media array trong task result.
            Returns: (is_done, is_failed, video_url, generation_status)
            """
            media_list = task_result.get("media", [])
            if not media_list:
                return False, False, None, ""
            for media_item in media_list:
                meta = media_item.get("mediaMetadata", {})
                gen_status = (meta.get("mediaStatus", {})
                              .get("mediaGenerationStatus", ""))

                if "SUCCESSFUL" in gen_status:
                    video_url = _deep_find_url(media_item)
                    return True, False, video_url, gen_status

                if "FAILED" in gen_status:
                    return False, True, None, gen_status

            # Return latest status for logging
            first_status = ""
            if media_list:
                first_status = (media_list[0].get("mediaMetadata", {})
                                .get("mediaStatus", {})
                                .get("mediaGenerationStatus", ""))
            return False, False, None, first_status

        while True:
            elapsed = time.time() - start_time
            # Stuck timeout
            if time.time() - last_status_change > STUCK_TIMEOUT:
                self._log(f"Video task stuck {STUCK_TIMEOUT}s, bỏ cuộc")
                break
            if self.local_server_url and attempt % 12 == 0:
                try:
                    health = requests.get(f"{self.local_server_url}/api/status", timeout=8)
                    if health.status_code == 200:
                        state = health.json()
                        server_state = str(state.get("server_state") or state.get("status") or "").lower()
                        accepting = bool(state.get("accepting_tasks", True))
                        if server_state == "down" or not accepting:
                            down_since = down_since or time.time()
                            if time.time() - down_since > 180:
                                msg = f"Local server unavailable for {int(time.time() - down_since)}s: state={server_state}, accepting={accepting}"
                                self._log(msg)
                                return False, VideoGenerationResult(
                                    status="failed", prompt=prompt, seed=seed,
                                    scene_id=scene_id, error=msg
                                ), msg
                        else:
                            down_since = None
                except Exception:
            ...
            try:
                ...
                if response.status_code != 200:
                    self._log(f"Video poll {attempt}: HTTP {response.status_code} ({int(elapsed)}s)")
                    time.sleep(poll_interval)
                    continue

                result = response.json()

                # --- Task chưa có kết quả (success=false, đang chờ) ---
                if not result.get("success"):
                    ...
                    if cur_status != last_status:
                        ...
                        last_status_change = time.time()
                    if attempt % 6 == 1:
                        self._log(f"Video poll {attempt}: waiting ({int(elapsed)}s) status={cur_status}")
                    time.sleep(poll_interval)
                    continue

                # --- Task có kết quả (success=true) ---
                task_result = result.get("result", {})
                ...
                cur_ops = task_result.get("operations", [])
                if cur_ops:
                    operations = cur_ops  # Lưu lại cho fallback
                    ...
                    if is_done and video_url:
                        ...
                        return True, VideoGenerationResult(
                            video_url=video_url,
                            operation_id=cur_ops[0].get("operation", {}).get("name"),
                            scene_id=scene_id, status="completed",
                            prompt=prompt, seed=seed
                        ), ""

                    if is_done and not video_url:
                        self._log(f"Video done but no URL in operations, checking media...")

                    if is_failed:
                        self._log(f"Video FAILED via operations: {err_msg[:200]}")
                        return False, VideoGenerationResult(
                            status="failed", prompt=prompt, seed=seed,
                            scene_id=scene_id, error=f"Video generation failed: {err_msg[:200]}"
                        ), f"Video generation failed: {err_msg[:200]}"

                # Check media array status (server đã poll Google xong)
                ...
                if is_done and video_url:
                    ...
                    op_name = ""
                    if cur_ops:
                        op_name = cur_ops[0].get("operation", {}).get("name", "")
                    return True, VideoGenerationResult(
                        video_url=video_url,
                ...
                if is_done and not video_url:
                    ...
                    if any_url:
                        ...
                        return True, VideoGenerationResult(
                            video_url=any_url, scene_id=scene_id,
                            status="completed", prompt=prompt, seed=seed
                        ), ""
                    self._log(f"Video done but no URL found anywhere. Result: {json.dumps(task_result)[:500]}")
                    return False, VideoGenerationResult(
                        status="failed", prompt=prompt, seed=seed,
                        scene_id=scene_id, error="Video completed but no URL in response"
                    ), "Video completed but no URL in response"

                if is_failed:
                    self._log(f"Video FAILED via media: {gen_status}")
                    return False, VideoGenerationResult(
                        status="failed", prompt=prompt, seed=seed,
                        scene_id=scene_id, error=f"Video generation failed: {gen_status}"
                    ), f"Video generation failed: {gen_status}"

                # Vẫn đang PENDING/PROCESSING — tiếp tục poll proxy
                status_str = gen_status or (cur_ops[0].get("status", "") if cur_ops else "")
                cur_combined = f"ops={len(cur_ops)}|{status_str}"
                if cur_combined != last_status:
                    last_status = cur_combined
                    last_status_change = time.time()
                    self._log(f"Video poll {attempt}: {status_str} ({int(elapsed)}s)")
                elif attempt % 6 == 1:
                    self._log(f"Video poll {attempt}: {status_str} ({int(elapsed)}s)")

                # Nếu chưa có result gì, cũng update status change
                time.sleep(poll_interval)
        ...
        # --- Proxy polling timeout — thử Google polling làm fallback ---
        if operations:
            self._log(f"Proxy polling timeout. Trying Google direct polling as fallback...")
            return self._poll_google_with_operations(
                operations, prompt, seed, scene_id,
                max_attempts=min(max_attempts, 30), poll_interval=poll_interval
            )

        return False, VideoGenerationResult(
            status="failed", prompt=prompt, seed=seed,
            scene_id=scene_id, error="Timeout waiting for video from proxy"
        ), "Timeout waiting for video from proxy"
    ...
    def _poll_google_with_operations(
        ...
        Chờ tối đa max_attempts * poll_interval giây (mặc định 10 phút).
        """
        url = f"{self.BASE_URL}/v1/video:batchCheckAsyncVideoGenerationStatus"
        self._log(f"Google video polling: {url} (max {max_attempts * poll_interval / 60:.0f} phút)")
        ...
        def _first_url(value: Any) -> Optional[str]:
            ...
            if isinstance(value, dict):
                for key in (
                    "fifeUrl", "signedUrl", "downloadUrl", "downloadURL",
                    "videoUrl", "videoURL", "url", "uri",
                ):
        ...
        def _extract_video_url(op: Dict[str, Any]) -> Optional[str]:
            operation = op.get("operation", {}) if isinstance(op, dict) else {}
            candidates = [
                operation.get("metadata", {}).get("video", {}) if isinstance(operation, dict) else {},
                operation.get("response", {}) if isinstance(operation, dict) else {},
                op.get("response", {}) if isinstance(op, dict) else {},
                op.get("result", {}) if isinstance(op, dict) else {},
                op,
            ]
        ...
        for attempt in range(max_attempts):
            try:
                poll_payload = {"operations": operations}
                response = requests.post(url, headers=google_headers, json=poll_payload, timeout=30)

                if response.status_code != 200:
                    self._log(f"Google poll {attempt+1}: HTTP {response.status_code}: {response.text[:200]}")
                    time.sleep(poll_interval)
                    continue
                ...
                if attempt % 6 == 0 or not ops:
                    self._log(f"Google poll {attempt+1}/{max_attempts}: ops={len(ops)}, data={json.dumps(result)[:200]}")
                ...
                # Cũng check done flag (một số response dùng done thay vì status)
                is_done = op.get("done", False)
                if attempt % 3 == 0 or is_done or "SUCCESSFUL" in status or "FAILED" in status:
                    self._log(f"Google poll {attempt+1}: status={status}, done={is_done}")

                # Check for success
                if status == "MEDIA_GENERATION_STATUS_SUCCESSFUL" or is_done:
                    video_url = _extract_video_url(op)
                    ...
                    if video_url:
                        ...
                        return False, VideoGenerationResult(
                            status="failed", prompt=prompt, seed=seed,
                            scene_id=scene_id, error="Video completed but response did not contain a downloadable URL"
                        ), "Video completed but response did not contain a downloadable URL"
