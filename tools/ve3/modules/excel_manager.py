...
if "reference_media_checked" not in CHARACTERS_COLUMNS:
    CHARACTERS_COLUMNS.append("reference_media_checked")
...
class Character:
    ...
    def __init__(
        ...
        is_child: bool = False,
        media_id: str = "",
        reference_media_checked: bool = False
    ):
        self.id = id
        self.role = role
        self.name = name
        ...
        self.reference_media_checked = reference_media_checked
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "reference_media_checked": self.reference_media_checked,
        }
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Character":
        ...
        checked_value = data.get("reference_media_checked", False)
        if isinstance(checked_value, bool):
            reference_media_checked = checked_value
        elif isinstance(checked_value, str):
            reference_media_checked = checked_value.lower() in ("true", "1", "yes", "done", "checked")
        else:
            reference_media_checked = bool(checked_value)
        ...
        return cls(
            ...
            reference_media_checked=reference_media_checked,
        )
...
class Scene:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "Scene":
        ...
        return cls(
            ...
            video_note=safe_str(data.get("video_note", "")),  # Ghi chú video
...
class PromptWorkbook:
    ...
    def _wait_for_save_window(self, timeout: float = 30.0, poll: float = 0.5) -> None:
        """Wait until another process finishes an in-progress atomic save."""
        import time
    ...
    def load_or_create(self) -> "PromptWorkbook":
        ...
        try:
            self._ensure_characters_sheet_schema()
        except Exception as e:
    ...
    def _get_sheet_headers(self, sheet_name: str) -> List[str]:
        """Lay header thuc te cua sheet de doc/ghi theo ten cot thay vi index cung."""
        if self.workbook is None:
            self.load_or_create()
        ws = self.workbook[sheet_name]
        return [cell.value for cell in ws[1]]

    def _row_to_dict_by_header(self, headers: List[str], row: tuple) -> Dict[str, Any]:
        data: Dict[str, Any] = {}
        for idx, header in enumerate(headers):
            if not header:
                continue
            data[str(header)] = row[idx] if idx < len(row) else None
        return data
    ...
    def _ensure_characters_sheet_schema(self) -> None:
        ...
        if self.workbook is None or self.CHARACTERS_SHEET not in self.workbook.sheetnames:
            return
        ...
        existing = {str(h) for h in headers if h}
        changed = False
        for column_name in CHARACTERS_COLUMNS:
            if column_name in existing:
                continue
            col_idx = ws.max_column + 1
            ws.cell(row=1, column=col_idx, value=column_name)
        ...
        if changed:
            self.logger.info("Updated characters sheet schema with missing columns")
        ...
        for item in pending:
            try:
                ...
                if wtype == 'scene':
                    ...
                    if thumb_id:
                        update_kw = {}
                        for k in ['img_path', 'img_path_portrait', 'status_img', 'status_portrait']:
                            if item.get(k):
                                update_kw[k] = item[k]
                        if update_kw:
                            self.update_thumbnail(int(thumb_id), **update_kw)
                            applied += 1
            except Exception as e:
    ...
    def get_characters(self) -> List[Character]:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            # Skip guide/instruction rows (HƯỚNG DẪN, ghi chú...)
            cell_id = str(row[0]).strip()
            if not cell_id or ' ' in cell_id or cell_id.startswith('-') or cell_id.startswith('HƯỚNG') or cell_id.startswith('#'):
                continue
    ...
    def get_scene_media_ids(self) -> Dict[str, str]:
        ...
        result = {}
        ws = self.workbook[self.SCENES_SHEET]
        headers = self._get_sheet_headers(self.SCENES_SHEET)
        header_map = {str(h): i + 1 for i, h in enumerate(headers) if h}
        headers = self._get_sheet_headers(self.SCENES_SHEET)
        header_map = {str(h): i + 1 for i, h in enumerate(headers) if h}

        # Tìm cột media_id
        media_id_col = header_map.get("media_id")
    ...
    def get_scenes(self) -> List[Scene]:
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            # Skip guide/instruction rows - scene_id phải là số
            try:
                int(row[0])
            except (ValueError, TypeError):
                continue

            data = self._row_to_dict_by_header(headers, row)
    ...
    def get_director_plan(self) -> List[Dict]:
        ...
        ws = self.workbook[self.DIRECTOR_PLAN_SHEET]
        headers = self._get_sheet_headers(self.DIRECTOR_PLAN_SHEET)
        plans = []
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            data = self._row_to_dict_by_header(headers, row)
            plans.append({
                "plan_id": data.get("plan_id") or data.get("scene_id") or row[0],
                "scene_id": data.get("scene_id") or data.get("plan_id") or row[0],
                "segment_id": data.get("segment_id", 1) or 1,
                "srt_start": data.get("srt_start", "") or "",
                "srt_end": data.get("srt_end", "") or "",
                "duration": data.get("duration", 0) or 0,
                "srt_text": data.get("srt_text", "") or "",
                "visual_moment": data.get("visual_moment", "") or "",
                "scene_kind": data.get("scene_kind", "") or "",
                "subject_mode": data.get("subject_mode", "") or "",
                "primary_subject": data.get("primary_subject", "") or "",
                "primary_action": data.get("primary_action", "") or "",
                "visual_anchor": data.get("visual_anchor", "") or "",
                "must_not_show": data.get("must_not_show", "") or "",
                "camera": data.get("camera", "") or "",
                "lighting": data.get("lighting", "") or "",
                "characters_used": data.get("characters_used", "[]") or "[]",
                "location_used": data.get("location_used", "") or "",
                "reference_files": data.get("reference_files", "[]") or "[]",
                "img_prompt": data.get("img_prompt", "") or "",
    ...
    def get_scene_planning(self) -> list:
        ...
        self._ensure_scene_planning_sheet()
        ws = self.workbook[self.SCENE_PLANNING_SHEET]
        headers = self._get_sheet_headers(self.SCENE_PLANNING_SHEET)
        ...
        for row in ws.iter_rows(min_row=2, values_only=True):
            ...
            plan = {
                ...
                "artistic_intent": data.get("artistic_intent", "") or "",
                "shot_type": data.get("shot_type", "") or "",
                "character_action": data.get("character_action", "") or "",
    ...
    def reset_flow_media_cache_if_project_changed(self, old_project_id: str, new_project_id: str) -> Dict[str, Any]:
        """Clear cached media/status fields when workbook switches to a new Flow project."""
        old_project_id = str(old_project_id or "").strip()
        new_project_id = str(new_project_id or "").strip()
        stats = {
            "changed": False,
            "characters_reset": 0,
            "scenes_reset": 0,
            "thumbnails_reset": 0,
            "pending_writes_cleared": False,
        }
        ...
        ws_chars = self.workbook[self.CHARACTERS_SHEET]
        char_headers = self._get_sheet_headers(self.CHARACTERS_SHEET)
        char_map = {str(h): i + 1 for i, h in enumerate(char_headers) if h}
        char_status_col = char_map.get("status")
        char_media_col = char_map.get("media_id")
        char_checked_col = char_map.get("reference_media_checked")
        char_child_col = char_map.get("is_child")
        if char_status_col and char_media_col:
            for row_idx in range(2, ws_chars.max_row + 1):
                raw_id = ws_chars.cell(row=row_idx, column=1).value
                if raw_id is None:
                    continue
                cell_id = str(raw_id).strip()
                if not cell_id or " " in cell_id or cell_id.startswith("-") or cell_id.startswith("HÆ¯á»šNG") or cell_id.startswith("#"):
                    continue
                changed = False
                if ws_chars.cell(row=row_idx, column=char_media_col).value not in (None, ""):
                    ws_chars.cell(row=row_idx, column=char_media_col, value="")
                    changed = True
                if char_checked_col and ws_chars.cell(row=row_idx, column=char_checked_col).value not in (None, "", False):
                    ws_chars.cell(row=row_idx, column=char_checked_col, value=False)
                    changed = True
                is_child = False
                if char_child_col:
                    raw_child = ws_chars.cell(row=row_idx, column=char_child_col).value
                    if isinstance(raw_child, bool):
                        is_child = raw_child
                    else:
                        is_child = str(raw_child or "").strip().lower() in ("true", "1", "yes")
                desired_status = "skip" if is_child else "pending"
                current_status = str(ws_chars.cell(row=row_idx, column=char_status_col).value or "").strip()
                if current_status != desired_status:
                    ws_chars.cell(row=row_idx, column=char_status_col, value=desired_status)
                    changed = True
                if changed:
                    stats["characters_reset"] += 1

        ws_scenes = self.workbook[self.SCENES_SHEET]
        scene_headers = self._get_sheet_headers(self.SCENES_SHEET)
        scene_map = {str(h): i + 1 for i, h in enumerate(scene_headers) if h}
        for row_idx in range(2, ws_scenes.max_row + 1):
            raw_scene_id = ws_scenes.cell(row=row_idx, column=1).value
            ...
            try:
                ...
                continue
            changed = False
            for key in ("media_id", "img_path", "video_path", "status_img", "status_vid"):
                col_idx = scene_map.get(key)
                if col_idx and ws_scenes.cell(row=row_idx, column=col_idx).value not in (None, ""):
                    ws_scenes.cell(row=row_idx, column=col_idx, value="")
                    changed = True
            if changed:
                stats["scenes_reset"] += 1

        if self.THUMBNAIL_SHEET in self.workbook.sheetnames:
            ws_thumb = self.workbook[self.THUMBNAIL_SHEET]
            thumb_headers = self._get_sheet_headers(self.THUMBNAIL_SHEET)
            thumb_map = {str(h): i + 1 for i, h in enumerate(thumb_headers) if h}
            for row_idx in range(2, ws_thumb.max_row + 1):
                if ws_thumb.cell(row=row_idx, column=1).value is None:
                    continue
                changed = False
                for key in ("img_path", "img_path_portrait", "status_img", "status_portrait"):
                    col_idx = thumb_map.get(key)
                    if col_idx and ws_thumb.cell(row=row_idx, column=col_idx).value not in (None, ""):
                        ws_thumb.cell(row=row_idx, column=col_idx, value="")
                        changed = True
                if changed:
                    stats["thumbnails_reset"] += 1

        pending_path = self._get_pending_path()
        if pending_path.exists():
            try:
                pending_path.unlink()
                stats["pending_writes_cleared"] = True
            except Exception as e:
                self.logger.warning(f"Cannot clear pending writes after project switch: {e}")

        stats["changed"] = any(
            (
                stats["characters_reset"],
                stats["scenes_reset"],
                stats["thumbnails_reset"],
                stats["pending_writes_cleared"],
            )
        )
        if stats["changed"]:
            self.logger.info(
                "Reset Flow media cache after project switch "
                f"({old_project_id[:8]}... -> {new_project_id[:8]}...): "
                f"chars={stats['characters_reset']}, scenes={stats['scenes_reset']}, "
                f"thumbs={stats['thumbnails_reset']}, pending={stats['pending_writes_cleared']}"
            )
        return stats

    def detect_existing_flow_assets(self) -> Dict[str, Any]:
        """Detect whether workbook already contains generated Flow assets/media."""
        stats = {
            "has_assets": False,
            "character_media": 0,
            "scene_media": 0,
        ...
        try:
            ...
            scene_map = {str(h): i + 1 for i, h in enumerate(scene_headers) if h}
            for key, stat_key in (
                ("media_id", "scene_media"),
                ("img_path", "scene_images"),
                ("video_path", "scene_videos"),
            ):
                col_idx = scene_map.get(key)
                if not col_idx:
                    continue
                for row_idx in range(2, ws_scenes.max_row + 1):
                    if ws_scenes.cell(row=row_idx, column=col_idx).value not in (None, ""):
                        stats[stat_key] += 1
        except Exception:
            pass
        ...
        try:
            if self.THUMBNAIL_SHEET in self.workbook.sheetnames:
                ...
                thumb_headers = self._get_sheet_headers(self.THUMBNAIL_SHEET)
                thumb_map = {str(h): i + 1 for i, h in enumerate(thumb_headers) if h}
                for key in ("img_path", "img_path_portrait"):
                    col_idx = thumb_map.get(key)
                    if not col_idx:
                        continue
                    for row_idx in range(2, ws_thumb.max_row + 1):
                        if ws_thumb.cell(row=row_idx, column=col_idx).value not in (None, ""):
                            stats["thumbnail_images"] += 1
        except Exception:
            pass

        stats["has_assets"] = any(
            stats[key] > 0
            for key in ("character_media", "scene_media", "scene_images", "scene_videos", "thumbnail_images")
        )
        return stats
