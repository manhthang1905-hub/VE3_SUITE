...
class ServerInfo:
    """Thong tin 1 server."""
    def __init__(self, url: str, name: str = "", enabled: bool = True, flow_account_name: str = ""):
        self.url = url.rstrip('/')
        self.name = name or url
        self.enabled = enabled
        self.flow_account_name = (flow_account_name or "").strip()
        self.queue_size = 0
    ...
    def __repr__(self):
        return (
            f"Server({self.name}, q={self.queue_size}, proc={self.processing_count}, "
            f"pending={self.local_pending}, state={self.server_state}, conn_fail={self.connect_fail_count})"
        )
...
class ServerPool:
    ...
    STATUS_STALE_GRACE = 90  # seconds - status loi tam, nhung ping con song
    ...
    def _parse_config(self, config: Dict):
            ...
            for s in server_list:
                if isinstance(s, str):
                    ...
                    self.servers.append(ServerInfo(
                        ...
                        flow_account_name=s.get('flow_account_name', ''),
                    ))
        else:
        ...
        self._log(
            ...
            + str([f"{s.name}/{s.flow_account_name or '-'}" for s in self.servers])
        )
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                with self._lock:
                    ...
                    server.processing_count = data.get('processing_count', 0)
                    server.load_count = data.get('load_count', server.queue_size + server.processing_count)
                    server.server_state = data.get('server_state', 'unknown')
                    server.chrome_ready = bool(data.get('chrome_ready', False))
                    server.accepting_tasks = bool(data.get('accepting_tasks', server.chrome_ready))
                    server.last_check = time.time()
                    server.last_response_time = response_time
                    # Server phan hoi OK → reset connection failures
                    server.connect_fail_count = 0
                    server.recent_timeout_count = 0
                return True
        except Exception:
            pass

        # /api/status co the fail tam thoi khi server dang recovery nang.
        # Thu /api/ping de phan biet server that su mat ket noi hay chi dang ban.
        try:
            ping = requests.get(f"{server.url}/api/ping", timeout=2)
            if ping.status_code == 200:
                ping_data = ping.json()
                snapshot_age = ping_data.get("snapshot_age")
                with self._lock:
                    server.last_response_time = 0.0
                    server.connect_fail_count = 0
                    server.recent_timeout_count += 1
                    if (
                        server.last_check > 0
                        and snapshot_age is not None
                        and snapshot_age <= self.STATUS_STALE_GRACE
                    ):
                        if server.server_state in ("unreachable", "unknown", "down"):
                            server.server_state = ping_data.get("server_state") or "status_delayed"
                        if server.server_state == "recovering":
                            server.accepting_tasks = True
                        return True
                    server.chrome_ready = False
                    server.accepting_tasks = False
                    server.server_state = "status_delayed"
                return False
        except Exception:
            pass

        # Refresh fail + ping fail = unreachable that
        with self._lock:
            server.chrome_ready = False  # v1.0.541: Khong ket noi duoc → khong ready
            server.accepting_tasks = False
            server.server_state = "unreachable"
            server.recent_timeout_count += 1
        return False
        ...
        with self._lock:
            ...
            for s in self.servers:
                if self._is_available(s):
                    ...
                    available.append(f"{s.name}(q={s.queue_size},proc={s.processing_count},state={s.server_state},rt={rt})")
                else:
                    if not s.enabled:
                        reason = "disabled"
                    elif s.last_check == 0:
                        reason = "not_checked"
                    elif s.server_state and s.server_state != "unknown":
                        reason = s.server_state
                    else:
                        reason = "connect_fail"
    ...
    def pick_best_server(self, auto_reserve: bool = True) -> Optional[ServerInfo]:
        ...
        with self._lock:
            ...
            candidates.sort(key=lambda s: (
                s.load_count + s.local_pending,       # Uu tien tong tai thuc te nho
                ...
                "servers": [
                    {
                        ...
                        "accepting_tasks": s.accepting_tasks,
