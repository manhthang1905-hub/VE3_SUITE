...
from agent.api.ve3_compat import router as ve3_compat_router, init_ve3_compat
...
async def run_ws_server():
    ...
    Retries binding if port is in TIME_WAIT (common on Windows after restart).
    """
    max_retries = 12
    for attempt in range(max_retries):
        try:
            server = await websockets.serve(
                ...
                ping_interval=None,  # Disable server pings — MV3 service worker sleeps
                ping_timeout=None,
            )
            logger.info("WebSocket server listening on ws://%s:%d", WS_HOST, WS_PORT)
            await asyncio.Future()  # run forever
        except OSError as e:
            if attempt < max_retries - 1 and e.errno in (10048, 98):  # EADDRINUSE (Win=10048, Linux=98)
                logger.warning("WS port %d in use (TIME_WAIT?), retry %d/%d in 5s...",
                               WS_PORT, attempt + 1, max_retries)
...
async def lifespan(app: FastAPI):
    ...
    init_ve3_compat(get_flow_client())
    ...
    if hasattr(signal, 'SIGTERM') and not sys.platform.startswith('win'):
...
app.include_router(ve3_compat_router)
...
async def health():
    ...
    return {
        ...
        "flowKeyPresent": bool(getattr(client, "_flow_key", None)),
        "ws": client.ws_stats,
    }
