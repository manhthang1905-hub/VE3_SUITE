...
class FlowClient:
    ...
    async def reset_captcha(self) -> dict:
        """Ask extension to clear Flow site data + refresh page to reset reCAPTCHA."""
        return await self._send("reset_captcha", {}, timeout=30)

    async def extract_project_id(self) -> dict:
        """Ask extension to extract project ID from the open Flow tab URL.

        Returns {"data": {"projectId": "uuid"}} on success,
        or {"error": "reason"} on failure.
        """
        return await self._send("extract_project_id", {}, timeout=15)
