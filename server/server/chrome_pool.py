...
class ChromePool:
    ...
    def __init__(self, log_callback: Callable = None):
        ...
        try:
            self._task_watchdog_timeout = max(60, int(os.getenv("CHROME_TASK_WATCHDOG_TIMEOUT", "1800")))
        except Exception:
            self._task_watchdog_timeout = 1800
    ...
    def _worker_loop(self, worker: ChromeWorker, task_queue: list,
        ...
        while True:
            ...
            def _watchdog():
                if not watchdog_stop.wait(self._task_watchdog_timeout):
                    with task_lock:
                        current = tasks.get(task_id)
                        if not current or current.get('status') != 'processing':
                            return
                        current['status'] = 'failed'
                        current['error'] = f"Task watchdog timeout after {self._task_watchdog_timeout}s"
                        current['completed_at'] = time.time()
                        stats['total_failed'] += 1
                    worker.total_failed += 1
                    worker.last_error = f"Watchdog timeout {self._task_watchdog_timeout}s"
                    worker.ready = False
                    self._log(f"[{worker_name}] WATCHDOG timeout: {task_id[:8]}... > {self._task_watchdog_timeout}s -> restart Chrome", "ERROR")
                    try:
                        if worker.session:
                            worker.session.close()
                    except Exception as close_err:
                        self._log(f"[{worker_name}] WATCHDOG close error: {close_err}", "WARN")
                    worker.session = None

            watchdog_thread = threading.Thread(
                target=_watchdog,
                daemon=True,
                name=f"ChromeWatchdog-{worker.index}-{task_id[:8]}",
            )
            watchdog_thread.start()

            with task_lock:
                tasks[task_id]['status'] = 'processing'
                tasks[task_id]['started_at'] = task_started_at
            ...
            try:
                ...
                with task_lock:
                    if tasks[task_id].get('status') == 'failed' and str(tasks[task_id].get('error', '')).startswith('Task watchdog timeout'):
                        result = {'error': tasks[task_id]['error']}
                    elif result and ('media' in result or 'operations' in result) and 'error' not in result:
                        ...
                        if err_str.startswith('Task watchdog timeout'):
                            self._log(f"[{worker_name}] FAIL (WATCHDOG): {task_id[:8]}... | {err_str}", "ERROR")
                            continue
                ...
                with task_lock:
                    ...
                    already_timed_out = bool(
                        ...
                        and str(current.get('error', '')).startswith('Task watchdog timeout')
                    )
